...
df["xsec_ggf13_h1"] = df.apply(lambda row: find_xsec(row['mh1']), axis=1)
df["xsec_ggf13_h2"] = df.apply(lambda row: find_xsec(row['mh2']), axis=1)
...
df["xsec_scaled_h1_2a1_4tau"] = df.h1ggrc2 * df.Brh1a1a1 * df.Bra1tautau * df.Bra1tautau
df["xsec_scaled_h2_2a1_4tau"] = df.h2ggrc2 * df.Brh2a1a1 * df.Bra1tautau * df.Bra1tautau
df["xsec_scaled_h2_2h1_4tau"] = df.h2ggrc2 * df.Brh2h1h1 * df.Bra1tautau * df.Bra1tautau

df["xsec_h1_2a1_4tau"] = df.xsec_ggf13_h1 * df.h1ggrc2 * df.Brh1a1a1 * df.Bra1tautau * df.Bra1tautau
df["xsec_h2_2a1_4tau"] = df.xsec_ggf13_h2 * df.h2ggrc2 * df.Brh2a1a1 * df.Bra1tautau * df.Bra1tautau
df["xsec_h2_2h1_4tau"] = df.xsec_ggf13_h2 * df.h2ggrc2 * df.Brh2h1h1 * df.Bra1tautau * df.Bra1tautau
...
# Note that in these plots, the size of the point is proprotional to the mass of $h_i$. It appears that $BR(h_1 \to a_1a_1)$ can have a whole range of values for $h_1 \to a_1a_1$ (where $h_1$ is often $h(125)$). For $h_2$, which often has $m \gg 125$ GeV, the BR is much larger and more consistent ($\sim 0.7$), for a wide range of $m_{h_2}$. (*Impact on kinematics - larger h2 -> larger boost, more collimated a1 decay products.*)
...
a.set_xlim(left=0.64, right=1)
a.set_ylim(bottom=1E-4, top=1E2)
a.set_yscale("log")
